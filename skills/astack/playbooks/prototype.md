# Prototype

1. Write down the specific question the prototype needs to answer — a
   design choice or empirical fork, not "explore the idea."
2. Build two or three competing versions, each the smallest thing that
   answers the question.
3. Compare them against the same real criteria — run them, measure them,
   look at them — not by reasoning about which one should be better.
   ([prove it against the real artifact](../principles.md#prove-it-against-the-real-artifact))
4. Throw away what didn't win. A prototype isn't meant to survive into the
   real implementation as-is.
   ([bias toward deletion](../principles.md#bias-toward-deletion))
5. Report: what question was settled, which option won, and why — the
   losing prototypes don't need to be preserved.
