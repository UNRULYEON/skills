...
Hit-area sizes and focus behavior are covered in `accessibility.md`; visual polish (radius, shadows, animation) in `ui.md`; line length and text spacing in `typography.md`.
...
Without an established density system, start with `12px` between adjacent bordered or filled controls and `24px` of clearance around borderless text- and icon-only controls. Compact layouts may use less when the accessibility hit-area rules (`accessibility.md`) do not overlap and the controls remain visually distinct.
