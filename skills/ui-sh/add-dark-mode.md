...
Use this when the user wants to add dark mode support to an existing UI. This
is the canonical dark-mode ruleset — `design.md`'s own dark-mode guideline
entry points here rather than restating it.
...
- brand-new design or layout work — see `design.md`
- standalone image dark-mode conversion only — see `dark-mode-image.md`
...
4. For each rasterized image that needs a dark-mode variant, follow `dark-mode-image.md`, which MUST load and use the `imagegen` skill before creating or editing image assets.
...
- Generate dark-mode raster image variants following `dark-mode-image.md`, which MUST load and use the `imagegen` skill before creating or editing any raster image assets
...
- Do not generate, edit, or replace raster image assets directly from this file; `dark-mode-image.md` owns that work and MUST use the `imagegen` skill.
- Require the `dark-mode-image.md` + `imagegen` handoff even when the image change seems simple, decorative, or incidental.
